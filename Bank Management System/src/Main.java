        ...
        while (true) {
            ...
            switch (a) {
                case 1:
                    ...
                    String[] mem_arr = new String[0];
                    int member=0;
                    if(account_type==3)
                    {
                        System.out.println("How many members do you want?");
                        member = scanner.nextInt();
                        mem_arr=new String[member];
                        for(int i=0;i<member;i++) {
                            System.out.println("Enter another name for the member:");
                            mem_arr[i]=scanner.next();
                        }
                    }
                    ...
                    int num=arrayList.get(arrayList.size() - 1).getAccount_number() + 1;
                    User u1 = new User(name, arrayList.get(arrayList.size() - 1).getAccount_number() + 1, 0, pin, true,account_ty);
                    if(account_type==3) {
                        for (int i = 0; i < member; i++)
                        {
                            User u=new User(mem_arr[i],num, 0, pin, true,account_ty);
                            arrayList.add(u);
                        }
                    }
                    arrayList.add(u1);
                    ...
                    if(account_type==3) {
                        for (int i = 0; i < member; i++)
                        {
                            System.out.println("Other joint account: "+mem_arr[i]);
                        }
                    }
                        ...
                        if (!checklist.isEmpty()) {
                            ...
                            if (ammount_add <= 0) {
                                ...
                                if(checklist.get(0).getPin()== pin_add) {
                                    ...
                                    if(Objects.equals(checklist.get(0).getAccount_type(), "Joint"))
                                    {
                                        List<User> joint_user=arrayList.stream().filter(user -> user.getAccount_number()==acc_num).peek(user -> user.setBalance(v)).collect(Collectors.toList());
                                    }
                                    else {
                    ...
                    try {
                        ...
                        if (!checklist.isEmpty()) {
                            ...
                            if (ammount_deposit > checklist.get(0).getBalance()) {
                                ...
                                if(Objects.equals(checklist.get(0).getAccount_type(), "Joint"))
                                {
                                    List<User> joint_user=arrayList.stream().filter(user -> user.getAccount_number()==acc_num).peek(user -> user.setBalance(val)).collect(Collectors.toList());
                                }
                                else{
                ...
                case 4:
                    try {
                        ...
                        String checkList_my_account_type=checkList_my.get(0).getAccount_type();
                        if (!checkList_my.isEmpty()) {
                            ...
                            String checkList_account_type=checkList.get(0).getAccount_type();
                            if (!checkList.isEmpty()) {
                                System.out.print("Enter the ammount you want to send=");
                                int bal = scanner.nextInt();
                                if (bal >= 0 && bal <= checkList_my.get(0).getBalance()) {
                                    if(Objects.equals(checkList_my_account_type, "Joint"))
                                    {
                                        List<User> joint_user=arrayList.stream().filter(user -> user.getAccount_number()==acc_num_my).peek(user -> user.setBalance(checkList_my.get(0).getBalance() - bal)).collect(Collectors.toList());
                                        checkList.get(0).setBalance(checkList.get(0).getBalance() + bal);
                                    }
                                    else if (Objects.equals(checkList_account_type,"Joint"))
                                    {
                                        checkList_my.get(0).setBalance(checkList_my.get(0).getBalance() - bal);
                                        List<User> joint_user=arrayList.stream().filter(user -> user.getAccount_number()==acc_num).peek(user -> user.setBalance(checkList.get(0).getBalance() - bal)).collect(Collectors.toList());
                                    }
                                   else {
                                        checkList.get(0).setBalance(checkList.get(0).getBalance() + bal);
                                        checkList_my.get(0).setBalance(checkList_my.get(0).getBalance() - bal);
